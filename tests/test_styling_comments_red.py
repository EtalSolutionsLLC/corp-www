...
class StylingCommentsRedTests(unittest.TestCase):
    ...
    def test_home_quick_summary_trigger_has_no_decorative_arrow_span(self):
        for root in (WWW, PRD):
            html = self._read(root, "index.html")
    ...
    def test_solutions_et_al_summary_copy_is_shorter(self):
        for root in (WWW, PRD):
            html = self._read(root, "index.html")
            partial = self._read(root, "partials/catalog-brands-full-list.html")
            data = json.loads(self._read(root, "catalogs/brands/items.json"))
            self.assertIn("Technology guidance for real operations.", html)
            self.assertIn("Technology guidance for real operations.", partial)
            ...
            self.assertNotIn("Focused technology guidance for real operational problems.", partial)

    def test_viewport_contract_contains_styling_overrides(self):
        for root in (WWW, PRD):
            css = self._read(root, "assets/css/viewport-contract.css")
